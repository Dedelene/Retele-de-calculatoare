...
class State:
    ...
    def list(self):
        with self.lock:
            if not self.data:
                return "Server state is empty."

            rezultat = []
            for cheie, valoare in self.data.items():
                rezultat.append(f"{cheie}: {valoare}")
    ...
    def pop(self, key):
        with self.lock:
            valoare_extrasa = self.data.pop(key, None)
            
            if valoare_extrasa is not None:
                return valoare_extrasa
            return "Key not found"
