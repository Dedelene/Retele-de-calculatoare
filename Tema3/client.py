...
class FTPClient:
    ...
    def rename_file(self):
        ...
        old_name = input("Introdu numele actual al fisierului: ").strip()
        new_name = input("Introdu noul nume: ").strip()
        command = {'command': 'rename_file', 'old_filename': old_name, 'new_filename': new_name}
        response = self.send_command(command)
    ...
    def read_file(self):
        print("\n📖 READ FILE (Server)")
        self.list_files()
        filename = input("Introdu numele fisierului pe care vrei sa-l citesti: ").strip()
    ...
    def download(self):
        print("\n📥 DOWNLOAD FILE")
        self.list_files()
        filename = input("Introdu numele fisierului pentru descarcare: ").strip()
        ...
        if response['status'] == 'error':
            ...
            try:
                ...
                print(f"✓ Fisierul '{filename}' a fost salvat in directorul local.")
            except Exception as e:
                print(f"✗ Eroare la salvarea locala: {str(e)}")
    
    def edit_file(self):
        print("\n🛠️  EDIT FILE (Server)")
        self.list_files()
        filename = input("Introdu numele fisierului pe care vrei sa-l editezi: ").strip()
        new_content = input("Introdu noul continut: ").strip()
    ...
    def see_file_operation_history(self):
        ...
        self.list_files()
        filename = input("Introdu numele fisierului pentru a-i vedea istoricul: ").strip()
        command = {'command': 'see_file_operation_history', 'filename': filename}
        response = self.send_command(command)
        if response['status'] == 'error':
            print(f"✗ {response['message']}")
        else:
            print(f"\n--- Istoric pentru {filename} ---")
            for actiune in response['history']:
                print(f"> {actiune}")
